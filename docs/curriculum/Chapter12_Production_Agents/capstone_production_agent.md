...
!!! tip "All-in-one capstone"
    A FastAPI agent with Redis memory, OpenTelemetry traces, evals, Dockerfile, docker-compose, and a Kubernetes manifest. The full source tree of every file in `Chapter12_Production_Agents/capstone_production_agent/` is embedded below — copy into a folder of the same name and run.
